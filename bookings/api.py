...
class BookingViewSet(viewsets.GenericViewSet):
    ...
    def create_booking(self, request):
        ...
        with transaction.atomic():
            ...
            overlap = room_locked.bookings.filter(date_start__lt=date_end, date_end__gt=date_start).exists()
            if overlap:
                return Response({'error': 'room not available for given dates'}, status=status.HTTP_409_CONFLICT)
            b = Booking.objects.create(room=room_locked, date_start=date_start, date_end=date_end)
